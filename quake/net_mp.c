...
int MPATH_Init (void)
{
	...
	((struct sockaddr_in *)&broadcastaddr)->sin_port = WSLIB_htons(net_hostport);
...
int MPATH_OpenSocket (int port)
{
	...
	if ( WSLIB_bind (newsocket, (void *)&address, sizeof(address)) == -1)
		goto ErrorReturn;
...
int MPATH_CloseSocket (int socket)
{
	...
	return WSLIB_closesocket (socket);
}
...
static int PartialIPAddress (char *in, struct qsockaddr *hostaddr)
{
	...
	((struct sockaddr_in *)hostaddr)->sin_addr.s_addr = (myAddr & WSLIB_htonl(mask)) | WSLIB_htonl(addr);
...
int MPATH_Read (int socket, byte *buf, int len, struct qsockaddr *addr)
{
	...
	ret = WSLIB_recvfrom (socket, (char *)buf, len, 0, (struct sockaddr *)addr, &addrlen);
	if (ret == -1)
	{
...
int MPATH_MakeSocketBroadcastCapable (int socket)
{
	...
	if (WSLIB_setsockopt(socket, SOL_SOCKET, SO_BROADCAST, (char *)&i, sizeof(i)) < 0)
		return -1;
...
int MPATH_Write (int socket, byte *buf, int len, struct qsockaddr *addr)
{
	...
	ret = WSLIB_sendto (socket, buf, len, 0, (struct sockaddr *)addr, sizeof(struct qsockaddr));
	if (ret == -1)
...
int MPATH_GetSocketAddr (int socket, struct qsockaddr *addr)
{
	...
	WSLIB_getsockname(socket, (struct sockaddr *)addr, &addrlen);
	a = ((struct sockaddr_in *)addr)->sin_addr.s_addr;
	if (a == 0 || a == WSLIB_htonl(INADDR_LOOPBACK))
