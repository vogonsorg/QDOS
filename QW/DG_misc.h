...
#if defined(__linux) || defined(__FreeBSD__) || defined(__NetBSD__) || defined(__OpenBSD__) || defined(__DJGPP__)
...
#if 0 // FS: DJGPP doesnt have readlink so I don't know what to do.
static void DG__SetExecutablePath(char* exePath)
{
...
#elif defined(__linux) || defined(__NetBSD__) || defined(__OpenBSD__) || defined(__DJGPP__)
	...
	char buf[PATH_MAX] = {0};
	int len;
#if defined(__linux) || defined(__DJGPP__)
	snprintf(buf, sizeof(buf), "/proc/%d/exe", getpid());
#else // the BSDs
	snprintf(buf, sizeof(buf), "/proc/%d/file", getpid());
#endif
	// readlink() doesn't null-terminate!
	len = readlink(buf, exePath, PATH_MAX-1);
	if (len <= 0)
	{
		// an error occured, clear exe path
		exePath[0] = '\0';
...
{
	size_t len;
	char* ret; // need one more byte for terminating 0
                            ...
                            const void* needle, size_t needlelen)
{
	...
	size_t hlen_for_needle_start;
	int n0;
	unsigned char* n0candidate;
...
/*
	...
	magic1 = (sizeof(uintptr_t) == 4) ? 0x01010101uL : 0x0101010101010101uLL;
	magic2 = (sizeof(uintptr_t) == 4) ? 0x80808080uL : 0x8080808080808080uLL;
#endif
*/
